...
```java
// 객체 정렬과 관련된 인터페이스: 
// Comparable: 클래스 내부에 객체 정렬 기준을 만들어놓을 떄 사용
// Comparator: 클래스 '외부에' " " " "  " " "
public class Writer implements Comparable<Writer> { // 제네릭에 비교대상을 넣는건데, 스스로 비교할거니까 Writer
	int num;
	String name;
	String bookTitle;
	public Writer(int num, String name, String bookTitle) {
		this.num = num;
		this.name = name;
	...
	public String toString() {
		return "Writer [num=" + num + ", name=" + name + ", bookTitle=" + bookTitle + "]";
	}
	
	
	// 정렬 기준을 나타내는 메서드
	@Override
	public int compareTo(Writer other) { // this와 other를 비교하는거임
		int result = this.num - other.num;
		return result; // 오름차순. this - other 순. (음수일 때 유지, 양수일 때 뒤집음)
	}
}
```
