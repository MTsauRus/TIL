...
- 모든 요소가 같고, 파라미터 종류만 달라야 함.
  - 즉, 리턴 타입이 다르다면 이는 오버로딩이 아니라 다른 메서드임
- 메서드 오버로딩의 예시
```java
void walk() {
...
3. 인스턴스 변수 명시적 초기화
    - ```java 
        class Class {
            int a = 10; //<- 인스턴스 생성 시 먼저 0 초기화 후 명시적으로 10 다시 초기화
        }
      ```
4. 생성자 호출
5. 참조값 반환
...
- ```java
    ...
    // 자식임
  ```
  - 메모리에 부모의 객체가 먼저 생성되고, 그 아래에 자식이 붙는 형식